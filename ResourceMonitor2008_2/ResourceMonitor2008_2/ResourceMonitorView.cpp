...
void CResourceMonitorView::InitTable()
{
	...
	for (vector<CString>::iterator iter = m_tableCaptions.begin() ; iter != m_tableCaptions.end(); iter++)
	{
...
void CResourceMonitorView::InitFrame()
{
	...
	int i = 1;
	for (vector<CString>::iterator iter = m_frameCaptions.begin(); iter != m_frameCaptions.end(); i++, iter++)
	{
